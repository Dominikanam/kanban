...
function moveNotes(array, sourceNoteId, targetNoteId) {
  ...
  arrayCopy.splice(targetIndex, 0, sourceIndex >= 0 ? arrayCopy.splice(sourceIndex, 1)[0] : sourceNoteId);
...
export default function lanes(state = initialState, action) {
  switch (action.type) {
    ...
    case MOVE_BETWEEN_LANES: {
      ...
      targetLane.notes = action.targetNoteId
        ? moveNotes(targetLane.notes, action.noteId, action.targetNoteId)
        : [...targetLane.notes, action.noteId];
